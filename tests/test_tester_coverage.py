...
def make_clubs():
    return [
        ...
        Club(name='Souths', home_field=NIHC),
...
class TestCheckForcedGames:
    """Tests for _check_forced_games: verifies FORCED_GAMES config is respected."""

    def test_no_forced_games_returns_empty(self):
        data = make_data(forced_games=[])
        games = [make_game('G1', 'Norths PHL', 'Wests PHL', 'PHL', 1, 1, '2026-05-10')]
        tester = DrawTester(make_draw(games), data)
        assert tester._check_forced_games() == []

    def test_forced_game_satisfied_no_violation(self):
        """A forced game with matching game in draw - no violation."""
        data = make_data(forced_games=[
            {'teams': ['Norths', 'Souths'], 'grade': 'PHL', 'date': '2026-05-10',
             'day': 'Sunday', 'description': 'Derby PHL'},
        ])
        games = [
            make_game('G1', 'Norths PHL', 'Souths PHL', 'PHL', 7, 7, '2026-05-10'),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_forced_games()
        assert len(violations) == 0

    def test_forced_game_missing_raises_violation(self):
        """No matching game for a forced entry - violation."""
        data = make_data(forced_games=[
            {'teams': ['Norths', 'Souths'], 'grade': 'PHL', 'date': '2026-05-10',
             'day': 'Sunday', 'description': 'Derby PHL'},
        ])
        # Game exists but wrong teams
        games = [
            make_game('G1', 'Tigers PHL', 'Wests PHL', 'PHL', 7, 7, '2026-05-10'),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_forced_games()
        assert len(violations) == 1
        assert 'Derby PHL' in violations[0].message
        assert 'Expected exactly 1' in violations[0].message

    def test_forced_game_duplicate_raises_violation(self):
        """Two matching games for a default (equal) forced entry - violation."""
        data = make_data(forced_games=[
            {'teams': ['Norths', 'Wests'], 'grade': 'PHL', 'date': '2026-06-12',
             'day': 'Friday', 'field_location': NIHC, 'description': 'NIHC Friday'},
        ])
        games = [
            make_game('G1', 'Norths PHL', 'Wests PHL', 'PHL', 12, 12, '2026-06-12',
                      day='Friday', time='19:00', field_location=NIHC),
            make_game('G2', 'Norths PHL', 'Wests PHL', 'PHL', 12, 12, '2026-06-12',
                      day='Friday', time='19:00', field_name='WF', field_location=NIHC),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_forced_games()
        assert len(violations) == 1
    ...
    def test_forced_game_lesse_zero_ok(self):
        """lesse constraint: 0 matching games is fine (at most 1)."""
        data = make_data(forced_games=[
            {'grade': 'PHL', 'date': '2026-05-17', 'day': 'Sunday',
             'field_location': NIHC, 'constraint': 'lesse',
             'description': 'SC weekend max 1'},
        ])
        tester = DrawTester(make_draw([]), data)
        violations = tester._check_forced_games()
        assert len(violations) == 0

    def test_forced_game_lesse_one_ok(self):
        """lesse constraint: 1 matching game is fine."""
        data = make_data(forced_games=[
            {'grade': 'PHL', 'date': '2026-05-17', 'day': 'Sunday',
             'field_location': NIHC, 'constraint': 'lesse',
             'description': 'SC weekend max 1'},
        ...
        games = [
            make_game('G1', 'Norths PHL', 'Tigers PHL', 'PHL', 8, 8, '2026-05-17',
                      field_location=NIHC),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_forced_games()
    ...
    def test_forced_game_lesse_two_violation(self):
        """lesse constraint: 2 matching games - violation."""
        data = make_data(forced_games=[
            {'grade': 'PHL', 'date': '2026-05-17', 'day': 'Sunday',
             'field_location': NIHC, 'constraint': 'lesse',
             'description': 'SC weekend max 1'},
        ...
        games = [
            make_game('G1', 'Norths PHL', 'Tigers PHL', 'PHL', 8, 8, '2026-05-17',
                      field_location=NIHC),
            make_game('G2', 'Wests PHL', 'Gosford PHL', 'PHL', 8, 8, '2026-05-17',
                      field_location=NIHC, day_slot=2),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_forced_games()
        assert len(violations) == 1
        assert 'at most 1' in violations[0].message

    def test_forced_game_greatere_satisfied(self):
        """greatere constraint: 1 or more games OK."""
        data = make_data(forced_games=[
            {'teams': ['Norths'], 'grade': 'PHL', 'day': 'Friday',
             'constraint': 'greatere', 'description': 'Norths at least 1 Friday'},
        ])
        games = [
            make_game('G1', 'Norths PHL', 'Tigers PHL', 'PHL', 5, 5, '2026-05-01',
    ...
    def test_forced_game_greatere_zero_violation(self):
        """greatere constraint: 0 matching games - violation."""
        data = make_data(forced_games=[
            {'teams': ['Norths'], 'grade': 'PHL', 'day': 'Friday',
             'constraint': 'greatere', 'description': 'Norths at least 1 Friday'},
        ])
    ...
    def test_forced_game_club_name_resolution(self):
        """Club names like 'Norths' resolve to 'Norths PHL' when grade is specified."""
        data = make_data(forced_games=[
            {'teams': ['Norths', 'Gosford'], 'grade': 'PHL', 'day': 'Friday',
             'field_location': CCHP, 'description': 'Norths vs Gosford at Gosford'},
        ])
        games = [
            make_game('G1', 'Gosford PHL', 'Norths PHL', 'PHL', 3, 3, '2026-04-17',
                      day='Friday', time='20:00', field_location=CCHP),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_forced_games()
        assert len(violations) == 0

    def test_forced_game_single_team_any_opponent(self):
        """Single team in 'teams' matches any opponent."""
        data = make_data(forced_games=[
            {'teams': ['Norths'], 'grade': 'PHL', 'date': '2026-07-24',
             'day': 'Friday', 'field_location': NIHC, 'description': 'Norths home Friday'},
        ])
        games = [
            make_game('G1', 'Norths PHL', 'Maitland PHL', 'PHL', 18, 18, '2026-07-24',
                      day='Friday', time='19:00', field_location=NIHC),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_forced_games()
    ...
    def test_forced_game_no_teams_any_game_in_scope(self):
        """No teams specified - any game matching the scope satisfies it."""
        data = make_data(forced_games=[
            {'grade': 'PHL', 'date': '2026-03-27', 'day': 'Friday',
             'field_location': CCHP, 'description': 'Gosford Friday Night'},
        ])
        games = [
            make_game('G1', 'Gosford PHL', 'Tigers PHL', 'PHL', 1, 1, '2026-03-27',
                      day='Friday', time='20:00', field_location=CCHP),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_forced_games()
    ...
    def test_forced_game_wrong_scope_no_match(self):
        """Game exists but doesn't match scope fields - violation."""
        data = make_data(forced_games=[
            {'teams': ['Norths', 'Wests'], 'grade': 'PHL', 'date': '2026-06-12',
             'day': 'Friday', 'field_location': NIHC, 'description': 'NIHC Friday'},
        ])
        # Game is at Gosford, not NIHC
        games = [
            make_game('G1', 'Norths PHL', 'Wests PHL', 'PHL', 12, 12, '2026-06-12',
                      day='Friday', time='20:00', field_location=CCHP),
        ]
    ...
    def test_forced_game_grades_plural(self):
        """Entry with 'grades' (plural) matches games in any listed grade."""
        data = make_data(forced_games=[
            {'teams': ['Norths', 'Souths'], 'grades': ['PHL', '2nd'],
             'date': '2026-05-10', 'day': 'Sunday', 'description': 'Derby multi-grade'},
        ])
        games = [
            make_game('G1', 'Norths PHL', 'Souths PHL', 'PHL', 7, 7, '2026-05-10'),
            make_game('G2', 'Norths 2nd', 'Souths 2nd', '2nd', 7, 7, '2026-05-10'),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_forced_games()
        # grades plural creates one entry covering both grades.
        # 2 games match, but constraint is equal (==1), so violation.
        # In practice, multi-grade forced games should use separate entries per grade.
        assert len(violations) == 1  # 2 games match, but equal expects 1

    def test_forced_game_severity_is_critical(self):
        """ForcedGames violations should be severity level 1 (CRITICAL)."""
        data = make_data(forced_games=[
            {'teams': ['Norths', 'Souths'], 'grade': 'PHL', 'date': '2026-05-10',
        ...
        violations = tester._check_forced_games()
        assert len(violations) == 1
        assert violations[0].severity_level == 1
        assert violations[0].severity == 'CRITICAL'


# ============== Blocked Games Check ==============

class TestCheckBlockedGames:
    """Tests for _check_blocked_games: verifies BLOCKED_GAMES config is respected."""

    def test_no_blocked_games_returns_empty(self):
        data = make_data(blocked_games=[])
        games = [make_game('G1', 'Norths PHL', 'Wests PHL', 'PHL', 1, 1, '2026-05-10')]
        tester = DrawTester(make_draw(games), data)
        assert tester._check_blocked_games() == []

    def test_blocked_game_respected_no_violation(self):
        """Blocked game with no matching game in draw - no violation."""
        data = make_data(blocked_games=[
            {'club': 'Souths', 'grade': 'PHL', 'date': '2026-05-22', 'day': 'Friday',
             'description': 'Souths PHL blocked May 22'},
        ])
        # Different team, so no match
        games = [
            make_game('G1', 'Norths PHL', 'Tigers PHL', 'PHL', 9, 9, '2026-05-22',
                      day='Friday'),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_blocked_games()
        assert len(violations) == 0

    def test_blocked_game_violated(self):
        """Game matching a blocked entry - violation."""
        data = make_data(blocked_games=[
            {'club': 'Souths', 'grades': ['PHL', '2nd'], 'date': '2026-05-24',
             'description': 'Souths PHL/2nd U18 SC'},
        ])
        games = [
            make_game('G1', 'Norths PHL', 'Souths PHL', 'PHL', 9, 9, '2026-05-24'),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_blocked_games()
        assert len(violations) == 1
        assert 'Souths PHL/2nd U18 SC' in violations[0].message
        assert 'G1' in violations[0].affected_games

    def test_blocked_game_no_teams_blocks_all_in_scope(self):
        """No teams/club in entry - blocks ALL games matching scope."""
        data = make_data(blocked_games=[
            {'grade': 'PHL', 'date': '2026-07-10', 'day': 'Friday',
             'field_location': NIHC, 'description': 'NIHC Friday blocked'},
        ])
        games = [
            make_game('G1', 'Norths PHL', 'Tigers PHL', 'PHL', 16, 16, '2026-07-10',
                      day='Friday', field_location=NIHC),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_blocked_games()
        assert len(violations) == 1

    def test_blocked_game_different_date_no_violation(self):
        """Game on different date than blocked - no violation."""
        data = make_data(blocked_games=[
            {'grade': 'PHL', 'date': '2026-07-10', 'day': 'Friday',
             'field_location': NIHC, 'description': 'NIHC Friday blocked'},
        ])
        # Same venue/grade/day but different date
        games = [
            make_game('G1', 'Norths PHL', 'Tigers PHL', 'PHL', 17, 17, '2026-07-24',
                      day='Friday', field_location=NIHC),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_blocked_games()
    ...
    def test_blocked_game_club_specific(self):
        """Block for specific club only matches that club's teams."""
        data = make_data(blocked_games=[
            {'club': 'Gosford', 'date': '2026-06-21',
             'description': 'Gosford recovery weekend'},
        ])
        # Gosford game on blocked date
        games = [
            make_game('G1', 'Gosford PHL', 'Norths PHL', 'PHL', 13, 13, '2026-06-21'),
            # Non-Gosford game on same date — should NOT be flagged
            make_game('G2', 'Norths PHL', 'Tigers PHL', 'PHL', 13, 13, '2026-06-21',
                      day_slot=2),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_blocked_games()
    ...
    def test_blocked_game_teams_pair(self):
        """Block specific team pair."""
        data = make_data(blocked_games=[
            {'teams': ['Norths', 'Souths'], 'grade': 'PHL', 'date': '2026-05-24',
             'description': 'Blocked pair'},
        ])
        games = [
            make_game('G1', 'Norths PHL', 'Souths PHL', 'PHL', 9, 9, '2026-05-24'),
            # Different pair on same date — should NOT be flagged
            make_game('G2', 'Tigers PHL', 'Wests PHL', 'PHL', 9, 9, '2026-05-24',
                      day_slot=2),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_blocked_games()
    ...
    def test_blocked_game_field_location_filter(self):
        """Block only applies at specified field_location."""
        data = make_data(blocked_games=[
            {'club': 'Norths', 'grade': 'PHL', 'day': 'Friday',
             'field_location': MAITLAND_PARK,
             'description': 'Norths blocked at Maitland Friday'},
        ])
        games = [
            # At Maitland - blocked
            make_game('G1', 'Maitland PHL', 'Norths PHL', 'PHL', 5, 5, '2026-05-01',
                      day='Friday', field_location=MAITLAND_PARK),
            # At NIHC - OK
            make_game('G2', 'Norths PHL', 'Wests PHL', 'PHL', 6, 6, '2026-05-08',
                      day='Friday', field_location=NIHC),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_blocked_games()
    ...
    def test_blocked_game_multiple_violations(self):
        """Multiple games matching a single block - multiple violations."""
        data = make_data(blocked_games=[
            {'grade': 'PHL', 'date': '2026-03-27', 'day': 'Friday',
             'field_location': NIHC, 'description': 'NIHC blocked'},
        ])
        games = [
            make_game('G1', 'Norths PHL', 'Tigers PHL', 'PHL', 1, 1, '2026-03-27',
                      day='Friday', field_location=NIHC),
            make_game('G2', 'Wests PHL', 'Gosford PHL', 'PHL', 1, 1, '2026-03-27',
                      day='Friday', field_location=NIHC, day_slot=2),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_blocked_games()
        assert len(violations) == 2

    def test_blocked_game_severity_is_critical(self):
        """BlockedGames violations should be severity level 1 (CRITICAL)."""
        data = make_data(blocked_games=[
            {'grade': 'PHL', 'date': '2026-07-10', 'day': 'Friday',
             'field_location': NIHC, 'description': 'Blocked'},
        ])
        games = [
            make_game('G1', 'Norths PHL', 'Tigers PHL', 'PHL', 16, 16, '2026-07-10',
                      day='Friday', field_location=NIHC),
    ...
    def test_blocked_game_grades_plural(self):
        """Entry with 'grades' (plural) matches games in any listed grade."""
        data = make_data(blocked_games=[
            {'club': 'Souths', 'grades': ['PHL', '2nd'], 'date': '2026-05-24',
             'description': 'Souths U18 SC'},
        ])
        games = [
            make_game('G1', 'Norths PHL', 'Souths PHL', 'PHL', 9, 9, '2026-05-24'),
            make_game('G2', 'Norths 2nd', 'Souths 2nd', '2nd', 9, 9, '2026-05-24',
                      day_slot=2),
            # 3rd grade Souths - should NOT be flagged
            make_game('G3', 'Norths 3rd', 'Souths 3rd', '3rd', 9, 9, '2026-05-24',
                      day_slot=3),
        ]
        tester = DrawTester(make_draw(games), data)
        violations = tester._check_blocked_games()
        assert len(violations) == 2  # G1 and G2 only
