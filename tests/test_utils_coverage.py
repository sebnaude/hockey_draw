...
class TestBuildForcedGameRules:
    ...
    def test_empty_forced_games(self, teams):
        rules, ctypes, _counts = _build_forced_game_rules([], teams)
        assert rules == {}
        assert ctypes == {}

    def test_none_forced_games(self, teams):
        rules, ctypes, _counts = _build_forced_game_rules(None, teams)
        assert rules == {}
        assert ctypes == {}
    ...
    def test_pair_forced_game(self, teams):
        ...
        rules, ctypes, _counts = _build_forced_game_rules(entries, teams)
        assert len(rules) == 1
