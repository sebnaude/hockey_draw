...
- different venue -> start times >= phl_2nd_cross_venue_min_minutes (150) apart.
...
class TestPHLAnd2ndAdjacency:
    ...
    def test_cross_venue_under_150_infeasible(self, phl_data):
        """Given Tigers PHL @ NIHC EF slot 3 (14:30) and Tigers 2nd @ Maitland
        slot 2 (13:30): different venues, |14:30-13:30| = 60 min < 150.
        Then INFEASIBLE. (spec-030: threshold dropped 180->150, so the prior
        150-min gap is now ALLOWED; this uses a genuine sub-150 gap.)"""
        model, X = build_model_X(phl_data)
        phl = _key(X, grade='PHL', team='Tigers PHL', week=1, day_slot=3,
                   field_name='EF', location=BROADMEADOW)  # 14:30
        sec = _key(X, grade='2nd', team='Tigers 2nd', week=1, day_slot=2,
                   field_name='Maitland Main Field', location=MAITLAND)  # 13:30
        # Oracle: 14:30 = 870 min, 13:30 = 810 min, |870-810| = 60 < 150.
        model.Add(X[phl] == 1)
        model.Add(X[sec] == 1)
        n = PHLAnd2ndAdjacency().apply(model, X, phl_data, _registry(model))
        assert n > 0
    ...
    def test_cross_venue_at_least_150_feasible(self, phl_data):
        """Given Tigers PHL @ NIHC EF slot 1 (11:30) and Tigers 2nd @ Maitland
        slot 3 (15:00): different venues, |15:00-11:30| = 210 min >= 150.
        Then FEASIBLE."""
        model, X = build_model_X(phl_data)
        phl = _key(X, grade='PHL', team='Tigers PHL', week=1, day_slot=1,
        ...
        # Oracle: 11:30 = 690 min, 15:00 = 900 min, |900-690| = 210 >= 150.
        model.Add(X[phl] == 1)
        model.Add(X[sec] == 1)
        PHLAnd2ndAdjacency().apply(model, X, phl_data, _registry(model))
        status, _ = solve_with_timeout(model)
