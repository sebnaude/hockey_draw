...
| 2 vs 2, per_matchup=1 (Tigers 5ths + University 5ths, 2 teams each) | 2×2 | 4 | 2 | 2 | 4 |
...
23. **`tests/atoms/test_club_vs_club_stacked_weekends.py`** rewritten with hand-computed oracles, no mocks:
    ...
    - 2×2 lower-grade pair (Tigers 5th × University 5th — the real season_test 2×2 pair: Tigers Yellow + Tigers Black vs University + University 2, per_matchup derived from season_test config): budget=max(2,2)×per_matchup, games/weekend=2. Hand oracle: 4 cross-team games land on exactly `budget` distinct Sundays, 2 games per Sunday, all 4 team-pairs covered. — (review note — Colts has only 1 team in 5th on season_test; Tigers-Colts 5th is a 2×1 pair, NOT 2×2. The real 2×2 pair is Tigers-University 5th. Test must use this pair or an explicit synthetic fixture labelled as synthetic.) —
...
- **Symmetric 2×2 case: multiple valid partitions.** For Tigers-University 5th (the real season_test 2×2 pair, per_matchup=1): 4 team-pairs, 2 aligned weekends, 2 games/weekend. The constraint system partitions the 4 team-pairs into 2 groups of 2 (one per weekend). There are 3 distinct such partitions; the solver picks one. The convenor's "free to choose" intent explicitly covers this case — no symmetry-breaking is needed or required. (review note — verified by reasoning: `sum_w team_pair_play[tp, w] == 1` for each of 4 team-pairs + `sum_{tp} team_pair_play[tp, w] == 2` on each of the 2 aligned weekends ⇒ each week gets exactly 2 of 4 team-pairs, remaining 2 go to the other week. Any of C(4,2)=6/2=3 partitions satisfies this. Correct and sufficient.)
