...
1. **Core-hard set defined and tagged `core_hard`** (stays hard in regen). The set, with the
   ...
   | `LockedPairings` + `ForcedGames` (the pins) | The freeze itself; soft pins would defeat regen. (forward-looking: `LockedPairings` is spec-025 machinery, not a current registry entry; wire this tag once spec-025 lands.) |
   Tag each `core_hard` in the registry (additional tag alongside its existing `core`/severity).
   (review fix — `LockedPairings` does not yet exist as a registry canonical name; flagged as forward-looking per spec-025 dependency.)
