...
- **Pinning is the soft-freeze half (spec-024).** `LOCKED_PAIRINGS` pins a pairing to its date,
  freeing time/slot/field. spec-025 *generates* these pins from the source draw. The config key
  is `data['locked_pairings']` (injected by `build_season_data`, per spec-024 DoD 2); the
...
- **Group selection has no current infrastructure hook.** The `--groups` CLI flag and
  ...
  constraint set. spec-025 does NOT add a `--groups` flag itself — that is spec-023's scope.
...
3. **Orchestration** (`run.py` `run_generate` → `main_staged`/`main_simple`): when
   ...
   - if spec-026 has landed (i.e. `resolve_groups(['regen'])` succeeds without ImportError/
     ...
     mechanism). If spec-026 has not landed (or spec-023 groups are not available), fall back
     to the normal full constraint selection and print a clear warning: "WARNING: spec-026 regen
...
- **Group selection requires spec-023.** Until spec-023 lands, `resolve_groups(['regen'])` does
  ...
  satisfy the full hard constraint set without spec-026's softening. The DoD 8 test (`--regen-
...
- **`data['locked_pairings']` concatenation order.** When both hand-authored config pins and
  auto-extracted regen pins exist, both are fed to spec-024's `generate_X` pass. If a pair
...
- **The `--groups` CLI flag and `resolve_groups()` infrastructure** — spec-023. spec-025 calls
