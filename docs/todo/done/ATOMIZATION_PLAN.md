...
Test baseline: **1287 passed, 1 skipped** after the diagnose port (1216 → 1352 → 1383 → 1285 → 1287; the dip at the second-to-last step reflects ~120 tests removed with `test_main_staged_coverage.py` + `test_severity_staged.py`, which were entirely dedicated to the deleted STAGES dicts).
