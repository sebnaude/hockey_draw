...
| spec-031 | Remove the `ClubFieldConcentration` tester-only diagnostic (no solver rule behind it; superseded by `ClubGameSpread`) | `spec-031-remove-club-field-concentration.md` | review_pending |
| spec-032 | Constraint group restructure: `symmetry_breakers` group (NIHC fill ×2 + lex ordering) applied by default in every solve with a `--no-symmetry-breakers` escape; `core` minus `EqualMatchUpSpacing`; lonesome `spacing` group | `spec-032-constraint-group-restructure.md` | review_pending |
...
| spec-034 | PENULTIMATE: green test suite + honest coverage (≥85% on atoms/registry/stages/tester) + three real-data no-mock assurances — atoms enforce on real data, `DrawTester` detects a failed constraint in a draw, soft constraints measured via `soft_pressure` | `spec-034-penultimate-green-suite.md` | review_pending |
| spec-035 | ULTIMATE: raw `--core` e2e solve on the forced-free `season_test` config (2026 base teams/fields, no forced games, week 1 NOT fixed, `--workers 10`) — get through presolve + survive ≥30 min of search (killed at 30 min), and read out remaining model symmetry (capture CP-SAT presolve stats; no historical baseline exists) | `spec-035-ultimate-e2e-core-run.md` | review_pending |
