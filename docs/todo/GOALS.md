...
| spec-034 | PENULTIMATE: green test suite + honest coverage (≥85% on atoms/registry/stages/tester) + three real-data no-mock assurances — atoms enforce on real data, `DrawTester` detects a failed constraint in a draw, soft constraints measured via `soft_pressure` | `done/spec-034-penultimate-green-suite.md` | done |
