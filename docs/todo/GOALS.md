...
| spec-030 | PHL/2nd cleanup: cross-venue gap 3 h → 2.5 h (`phl_2nd_cross_venue_min_minutes` 180→150); delete the redundant `PHLAnd2ndConcurrencyAtBroadmeadow` atom (subsumed by `PHLAnd2ndAdjacency`); locked-week skip on `PHLConcurrencyAtBroadmeadow` (predecessor parity) | `spec-030-phl-2nd-cleanup.md` | review_pending |
