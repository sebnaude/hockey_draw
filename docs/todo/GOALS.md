...
| spec-009 | FORCED count rules + adjusters — end-to-end verification | `done/spec-009-forced-count-rules-verification.md` | done |
...
| spec-023 | Constraint groups: composable, deduped, flag-selected (`--groups`); a constraint is applied whole (hard+soft together); delete the `soft_only` hard/soft machinery (supersedes the rejected atom-hard-soft-phases design) | `done/spec-023-constraint-groups.md` | done (2026-05-24) |
...
| spec-030 | PHL/2nd cleanup: cross-venue gap 3 h → 2.5 h (`phl_2nd_cross_venue_min_minutes` 180→150); delete the redundant `PHLAnd2ndConcurrencyAtBroadmeadow` atom (subsumed by `PHLAnd2ndAdjacency`); locked-week skip on `PHLConcurrencyAtBroadmeadow` (predecessor parity — already provided by `iter_phl_keys`, pinned by regression test) | `done/spec-030-phl-2nd-cleanup.md` | done |
| spec-031 | Remove the `ClubFieldConcentration` tester-only diagnostic (no solver rule behind it; superseded by `ClubGameSpread`) | `done/spec-031-remove-club-field-concentration.md` | done |
| spec-032 | Constraint group restructure: `symmetry_breakers` group (NIHC fill ×2 + lex ordering) applied by default in every solve with a `--no-symmetry-breakers` escape; `core` minus `EqualMatchUpSpacing`; lonesome `spacing` group | `done/spec-032-constraint-group-restructure.md` | done |
| spec-033 | Remove `ClubVsClubAlignment` slack (dead in solver, live+divergent in tester) + audit soft-analogue coverage of slack-tolerant constraints; close the one gap by adding a normal-mode `BalancedByeSpacing` soft analogue | `spec-033-slack-cleanup-soft-analogue-audit.md` | review_pending |
| spec-034 | green test suite + honest coverage (≥85% on atoms/registry/stages/tester) + three real-data no-mock assurances — atoms enforce on real data, `DrawTester` detects a failed constraint in a draw, soft constraints measured via `soft_pressure` | `done/spec-034-penultimate-green-suite.md` | done |
| spec-035 | raw `--core` e2e solve on the forced-free `season_test` config (2026 base teams/fields, no forced games, week 1 NOT fixed, `--workers 10`) — get through presolve + survive ≥30 min of search (killed at 30 min), and read out remaining model symmetry (capture CP-SAT presolve stats; no historical baseline exists) | `spec-035-ultimate-e2e-core-run.md` | review_pending |
| spec-039 | Machine-checked registry↔tester alignment gate: a standing test asserting every atom's `tester_check_methods`/`slack_key`/`has_soft_component` stays truthful against live `DrawTester` + `run.py` slack set + `PENALTY_WEIGHTS`; new `no_tester_check_reason` field labels deliberately-unchecked soft atoms; `analytics/registry_audit.py` produces the shared `AlignmentReport` | `spec-039-registry-tester-alignment-gate.md` | review_pending |
| spec-040 | Slack & limit provenance: persist the effective constraint config (CLI `constraint_slack` + base `constraint_defaults`) into draw/checkpoint metadata; `analytics/slack_provenance.py` resolver reconstructs the slack a draw was solved at (stored provenance, else current-config fallback + WARNING); `DrawTester` auto-replays faithfully so analysis stops throwing false violations | `spec-040-slack-limit-provenance-faithful-replay.md` | review_pending |
| spec-041 | Soft-constraint outcome measurement: per-rule 0–100 satisfaction score + raw domain metric recomputed from draw geometry (penalty values aren't persisted), incl. the unmeasured atoms (Maitland away-on-NRL-weekends `PreferredWeekendsAwayGround`, soft `AwayClubHomeWeekendsCount`); per-team + per-club rollups; measured at the draw's effective slack | `spec-041-soft-constraint-outcome-measurement.md` | review_pending |
| spec-042 | Auto-generated draw analysis report (HTML + JSON sidecar + Plotly): assembles alignment/slack-provenance/hard-violations/soft-scores into per-team/club/rule breakdowns with smart dense layouts; auto-runs at end of `save_solver_output` (non-fatal) + `report --analysis` CLI; JSON `schema_version` is the forward contract for the eventual UI | `spec-042-auto-analysis-report.md` | review_pending |
