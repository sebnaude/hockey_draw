...
| 4 — FORCED/BLOCKED count adjusters | 🟡 FRAMEWORK SHIPPED (`1521c9b`); 5 adjuster formulas proposed in `docs/COUNT_ADJUSTERS.md` awaiting user sign-off before per-adjuster implementations land. Engine calls `run_count_adjusters(data)` during `build_groupings`; no actual adjusters registered yet. | partial |
