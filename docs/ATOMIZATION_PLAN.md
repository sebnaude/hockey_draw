...
| 7a — Tests on real sampled data | 🟡 INITIAL DELIVERY — 4 violation fixtures (`tests/fixtures/violations/`) covering NoDoubleBookingTeams/Fields, ClubGradeAdjacency, MaxMaitlandHomeWeekends + walking test in `tests/test_violation_fixtures.py`. `ViolationBreakdown` dataclass + `ViolationReport.breakdown` property + 7 unit tests in `tests/test_violation_breakdown.py`. `Violation` gains `affected_clubs` and `metric_value` fields for soft-pressure rollups. Future fixtures can be added by dropping a JSON in the directory. | shipped |
| 7b — Configurable stages | 🟡 FOUNDATION — `DEFAULT_STAGES` in `config/defaults.py` lists 5 named stages keyed by canonical atom name. `constraints/stages.py::load_solver_stages`/`validate_solver_stages`/`list_stages` provide the loading + validation API. 10 new tests. `docs/STAGES.md` describes the schema. **Remaining (final-final push):** wire `main_staged.py` to read `data['solver_stages']` and dispatch via the registry; add CLI flags `--stages-config` / `--stage-only` / `--skip-stage` / `--list-stages`; add `_validate_stages` phase to `validate_game_config`; delete `STAGES`/`STAGES_AI` dicts. | foundation shipped |
| 7c — Move legacy to `constraints/archived/` | ⬜ DEFERRED to final-final push — risk-managed to avoid touching ~20 import sites in this session. The atoms own the dispatch path on `final-form` (legacy `_phl_times_*`, `_club_day_*`, `_club_alignment_*` etc. methods retained as parity reference but not invoked). **Remaining:** move 3 files into `constraints/archived/`; update every prod import site to go via the registry; add `tests/test_no_legacy_imports.py`; remove `--ai` CLI flag. | pending |
| 7d — Documentation update | ✅ DONE (this push) — `docs/ATOMIZATION_PLAN.md` (this), `docs/COUNT_ADJUSTERS.md`, `docs/ATOMIZATION_HANDOFF.md`, `docs/CONSTRAINT_INVENTORY.md`, `docs/STAGES.md` (new), `docs/HARNESS.md` (new), `README.md`, `CLAUDE.md` — all updated. | shipped |

Test baseline at this point: **1305 passed, 1 skipped** (started at 1216; 1272 after 3a; 1287 after 3b; 1298 after 3c; 1305 after Phase 4 framework; 1323 after Phase 4 adjusters; 1329 after Phase 6; ~1356 after Phase 7a/7b foundation).
...
Wired via `UnifiedConstraintEngine._club_day_atoms_hard`. Legacy
`_club_day_scheduling` + `_club_day_field_contiguity` retained as parity
