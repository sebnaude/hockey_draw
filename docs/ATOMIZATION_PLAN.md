...
| 7b — Configurable stages | ✅ DONE — `DEFAULT_STAGES` in `config/defaults.py` + `constraints/stages.py` API + `apply_solver_stage` dispatcher + `StagedScheduleSolver.run_solver_stages_solve` driving the per-stage loop. CLI flags `--stages-config`, `--stage-only`, `--skip-stage`, `--list-stages`. Phase 22 `_validate_stages` step in `validate_game_config`. 28 tests across `test_solver_stages.py` + `test_solver_stages_dispatch.py`. **Note:** legacy `STAGES`/`STAGES_AI` dicts in `main_staged.py` remain in place — the new path runs by default for non-severity solves; severity dispatch still uses `STAGES_SEVERITY[_AI]`. | `dd76a79` |
| 7c — Move legacy to `constraints/archived/` | 🟡 PARTIAL — `constraints/archived/` package skeleton (`__init__.py` + `README.md`) shipped. `tests/test_no_legacy_imports.py` enforces "no prod import of `constraints.archived.*`". **Remaining:** physically move `constraints/{original,ai,archived_equalspacing_original}.py` into the archived package; rewrite `STAGES_SEVERITY[_AI]` consumers + `run_list_constraints` + `run_diagnose` to use the registry; remove `--ai` CLI flag; tighten the lockdown test to also forbid `constraints.original` / `constraints.ai`. See `docs/ATOMIZATION_HANDOFF.md` for the deferred plan. | `4599f01` |
| 7d — Documentation update | ✅ DONE — `docs/ATOMIZATION_PLAN.md`, `docs/COUNT_ADJUSTERS.md`, `docs/ATOMIZATION_HANDOFF.md`, `docs/CONSTRAINT_INVENTORY.md`, `docs/STAGES.md`, `docs/HARNESS.md`, `README.md`, `CLAUDE.md` — all current. | shipped |

Test baseline: **1383 passed, 1 skipped** (1216 → 1352 → 1383 across the chain; +31 from this push covering the new dispatcher, CLI flags, validation, fixtures, and metadata).
