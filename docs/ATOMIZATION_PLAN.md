...
| 7b — Configurable stages | 🟡 FOUNDATION — `DEFAULT_STAGES` in `config/defaults.py` lists 5 named stages keyed by canonical atom name. `constraints/stages.py::load_solver_stages`/`validate_solver_stages`/`list_stages` provide the loading + validation API. 10 new tests. The `main_staged.py` dispatch still uses the legacy hardcoded `STAGES` / `STAGES_AI` dicts; rewiring to read `data['solver_stages']` and the `--stages-config` / `--stage-only` / `--skip-stage` / `--list-stages` CLI flags are deferred (see final summary). | shipped (foundation only) |
| 7c — Move legacy to `constraints/archived/` | ⬜ DEFERRED — risk-managed to avoid touching ~20 import sites in this session. The atoms own the dispatch path on `final-form` (legacy `_phl_times_*`, `_club_day_*`, `_club_alignment_*` etc. methods are retained as parity reference but not invoked). Move + lockdown tracked for a follow-up. | deferred |
| 7d — Documentation update | ✅ DONE (final-push pass) — `docs/ATOMIZATION_PLAN.md` (this), `docs/COUNT_ADJUSTERS.md`, `docs/ATOMIZATION_HANDOFF.md` all reflect shipped phases; CLAUDE.md atom-aware notes remain current. | shipped |
