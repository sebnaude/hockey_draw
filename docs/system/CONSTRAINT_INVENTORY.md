...
| BalancedByeSpacing | bye_spacing |
...
| BalancedByeSpacing | constraints/atoms/balanced_bye_spacing.py (spec-008 Part B; spec-033 Unit B) | HARD: for each team in each grade with `byes_per_team >= 2`, per-round bye-indicator BoolVar `B_t_r := 1 - sum(team_vars_r)`. Forbid `B_t_r1 + B_t_r2 <= 1` for every pair of rounds with `r2 - r1 <= S`, where `S = max(0, ideal_bye_gap(R, byes) - base_slack - config_slack)` and `ideal_bye_gap(R, b) = max(0, R//b - 1)`. Skips (locked, locked) pairs. Own slack key `BalancedByeSpacing` and own base-slack `CONSTRAINT_DEFAULTS['bye_spacing_base_slack']` so convenor can loosen byes independently of matchup spacing. **spec-033 Unit B: (a) SOFT analogue (normal mode) — reusing the SAME `B_t_r` indicators, penalises bye pairs in the band `(S, S_base]` (`S_base = ideal_bye_gap`) via a penalty BoolVar `p ≥ B_r1 + B_r2 - 1` appended to `data['penalties']['BalancedByeSpacing']` (weight `PENALTY_WEIGHTS['BalancedByeSpacing']`=100_000), pushing byes toward the ideal without relaxing the hard floor; (b) base slack 0→2 so the hard floor sits 2 below the raw ideal (the raw ideal floor risked infeasibility); (c) peeled from `core`/`critical_feasibility` into its own lonesome `bye_spacing` group (mirrors EqualMatchUpSpacing→`spacing`), `_is_fresh_build` widened so it still reaches `default`/`all`/`production`; severity_level=2 and slack_key unchanged; NOT in `regen` (its `BalancedByeSpacingRegenSoft` analogue covers regen).** Soft analogue ❌→✅; group core→bye_spacing; base slack 0→2. | 2 | BalancedByeSpacing | BalancedByeSpacing (atom; spec-033 adds normal-mode soft + base slack 2 + `bye_spacing` group) |
