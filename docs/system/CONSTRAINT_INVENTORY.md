...
Generated against `final-form` and updated through spec-002 + spec-003 + spec-004 + spec-006 + spec-007. The registry currently has **45 entries**: 21 originals + 5 PHL atoms (3a) + 5 ClubDay atoms (3b) + 4 ClubVsClub atoms (3c) + 2 Phase-6 generic aliases (`NonDefaultHomeGrouping`, `AwayAtNonDefaultGrouping`) + 1 spec-002 soft-only penalty atom (`SoftLexMatchupOrdering`) + 2 spec-007 atoms (`SameGradeSameClubNoConcurrency`, `TeamPairNoConcurrency`) + 2 spec-003 atoms (`NIHCFillWFBeforeEF`, `NIHCFillEFBeforeSF`) + 1 spec-006 soft-only penalty atom (`PreferredWeekendsAwayGround`) + 2 spec-004 atoms (`AwayClubHomeWeekendsCount`, `AwayClubPerOpponentAndAggregateHomeBalance`).
